...
; KOF96 lacks KOF95's short B,D motion descriptor used by Annu Mutsube.
MoveInput_Nakoruru_BD95:
	db $02
	db KEY_DOWN, KEY_DOWN, $01, $14
	db KEY_RIGHT, KEY_RIGHT, $01, $FF

; Play_LoadProjectileOBJInfo clears the extra-object slots after the pre-round
; animations. Spawn persistent KOF95 helpers only after that clear, then enter
; KOF96's normal gameplay loop. This supports Nakoruru on either side.
MixKOF_StartPlayWithPersistentHelpers:
...
MoveInputReader_Nakoruru:
	mMvIn_Validate Nakoruru
.chkAir:
	;             SELECT + B                           SELECT + A
	mMvIn_ChkEasy MoveInit_Nakoruru_ElerushKamuiRimse, MoveInit_Nakoruru_YatoroPokku
	mMvIn_ChkGA Nakoruru, .chkAirPunch, MoveInputReader_Nakoruru_NoMove
...
.chkGround:
	; Easy Move review against KOF95 bank16: preserve the two air shortcuts,
	; while ground routes follow the original command families and keep F/B
	; distinct. Elerush Kamui Rimse is the only super.
	mMvIn_ChkEasyDir MoveInit_Nakoruru_AmubeYatoro, MoveInit_Nakoruru_LelaMutsube, MoveInit_Nakoruru_AnnuMutsube, MoveInit_Nakoruru_KamuiRimse, MoveInit_Nakoruru_MamahahaFlight, MoveInit_Nakoruru_ElerushKamuiRimse, MoveInputReader_Nakoruru_NoMove
	mMvIn_ChkGA Nakoruru, .chkPunch, .chkKick
...
ENDC
	...
	mMvIn_ChkDir MoveInput_Nakoruru_BD95, MoveInit_Nakoruru_AnnuMutsube
